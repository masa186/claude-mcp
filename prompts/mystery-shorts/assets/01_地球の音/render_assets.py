#!/usr/bin/env python3
"""
「今も正体が分かっていない 地球の音 3選」用の図版レンダラ。

【使いどころを間違えないこと】
参考動画6本を解析した結果、この系統は最初から最後まで写真的な絵で構成されており、
データビジュアルは1カットも使われていない。図版は「効かせどころ」だけに使う。

  既定で描くのは3点だけ（カット6・7・32、および34-35用の比較図）。
  この3点は、ナレーションが述べている内容そのものが図であるカット。
    カット6-7「周波数が上昇していく音が延々と繰り返される」→ 上昇掃引そのもの
    カット32 「52ヘルツ」                                  → ピークそのもの
  それ以外のカットに図版を当てると、黒背景のグラフが続いて確実に飽きられる。

  --all を付けると、没にした図版（オシロスコープ・季節変動グラフ・ハム波形・
...
共通処理：中央のテロップ帯を暗く落とす／ビネット／フィルムグレイン／文字は一切入れない
  ...
  python3 render_assets.py --all    # 没にした分も含めて全部
"""
