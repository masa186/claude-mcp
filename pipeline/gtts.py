...
#
# 空にしてある。読み方ごとに違う指示を投げると、同じ声の名前（Charon）でも
# モデルが別の人格を作ってしまい、1本の中で声そのものが変わる。
# 同じモデル・同じ声名のまま、地の文8行とフリ6行を測ったら、
# 読み方どうしの中心の距離 5.04 に対して読み方の中でのばらつきは 2.73 で、
# 読み方ではっきり分かれていた。「3種類の男の声が混ざっている」の正体はこれ。
#
# フリとオチの差は、合成の指示ではなく、後から自分で付ける
# （間・速さ・音量。voice.py の PAUSE / HOLD / RATE / LEVEL）。
# 伸びているショート41本を聞いて数えた上位も、声が大きい33本・
# 直前に間32本・ゆっくり17本で、どれも合成に頼らずに作れるものだった。
#
# 下の2つは残してある。1話ぜんぶを1つの指示で録り直せる枠がある日に、
# 「指示で分ける」と「後から付ける」を並べて比べるため。
STYLES = {}
