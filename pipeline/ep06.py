...
# 1ショット1.76秒に詰めたので、要素の出し方も詰める。
# 既定（STAGGER 0.40／POP 0.22）だと、板の文字が出そろうまで0.65秒かかり、
# カットの頭に「何も無い黒板」が3〜6コマ入る。実測でも、図が変わる境目
# （kiku→pyre）のコマ間の差が3.7しかなく、カットとして画に出ていなかった。
render.STAGGER = 0.13
render.POP = 0.08

# 参考6本の画面の埋まりは66〜85%、こちらは30%だった。内訳を測ると
# board のショットが11.6%（face は50.4%）で、黒板の緑が空いていた。
# 図と文字の下に一段暗い板を敷いて埋める。
render.CARD_ON = True
