...
# 10. 誤った説明：上下の空気が同時にゴールする -------------------------
c = Canvas(940, 460)
c.tri((120, 250), (720, 210), (720, 268))          # 翼（横から）
c.bar(720, 210, 820, 246, 12)
for y, lab in ((150, 'up'), (350, 'dn')):
    c.arrow(90, y, 800, y, 10, CHALK, 40)
c.disc(830, 150, 22, HEAT, 0.9)
c.disc(830, 350, 22, HEAT, 0.9)
c.bar(806, 126, 854, 174, 11, HEAT)                # ×
c.bar(854, 126, 806, 174, 11, HEAT)
c.render(OUT + '/10_wing_wrong.png')

# 11. 正しい説明：空気を下へ曲げ、その反作用で翼が上がる ----------------
c = Canvas(940, 520)
c.tri((140, 250), (700, 214), (700, 272))
c.bar(700, 214, 800, 250, 12)
for y in (130, 180):
    c.arrow(70, y, 560, y, 9, CHALK, 34)
for sx, sy in ((600, 300), (700, 330), (800, 360)):  # 下へ曲がる空気
    c.arrow(sx - 120, sy - 60, sx, sy, 10, CHALK, 36)
c.arrow(400, 240, 400, 90, 16, HEAT, 56)             # 揚力
c.render(OUT + '/11_wing_lift.png')
